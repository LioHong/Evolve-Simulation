# https://github.com/mmtechslv/nwunch/blob/master/nwunch.py
import numpy as np
import pandas as pd
...
def print_aln_details(aln): # Function to print steps of particular aln
    ...
    arrow_dict = {'1':'\u2190','2':'\u2196','3':'\u2191'}
    for elem in aln[2]:
        print('Step: '+str(elem[0])+' -> Score = ' + str(elem[1])    + ' -> Dir: ' + (arrow_dict[elem[2]] if elem[2] in arrow_dict else 'Error'))
    print('\n-------------Final aln-------------\n')
    # print(aln[1]+'\n'+aln[0])
    print(aln[0])
    return
...
def print_all(ALIGNMENTS): #Function to print everything
    ...
    print('Overall Score: '+str(ALIGNMENTS[0][2][0][1])+'\n')
    for almt in ALIGNMENTS:
        print_aln_details(almt)
    return
...
for elem in ALN_PATHWAYS:
    ...
    # aln_df = pandalign(top_aln,side_aln)
    aln_df = pandalign(side_aln,top_aln)
    # ALIGNMENTS.append([top_aln[::-1],side_aln[::-1],elem,aln_info,aln_count])
    ALIGNMENTS.append([aln_df,elem,aln_info,aln_count])

# print_alns_only(ALIGNMENTS)
print_all(ALIGNMENTS)
